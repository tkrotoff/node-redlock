...
${(await Promise.all(error.attempts))
        .map((s, i) => `ATTEMPT ${i}: ${formatWithOptions({ colors: true }, {
                ...
                for (const e of await Promise.allSettled(error.attempts)) {
                    ...
                    if (e.status === "fulfilled") {
                        for (const v of (_b = (_a = e.value) === null || _a === void 0 ? void 0 : _a.votesAgainst) === null || _b === void 0 ? void 0 : _b.values()) {
                            t.assert(v instanceof ResourceLockedError, "The error was of the wrong type.");
...
run("instance", new Redis({ host: "redis-multi-instance-a" }), new Redis({ host: "redis-multi-instance-b" }), new Redis({ host: "redis-multi-instance-c" }));
run("cluster", new Cluster([{ host: "redis-multi-cluster-a-1" }]), new Cluster([{ host: "redis-multi-cluster-b-1" }]), new Cluster([{ host: "redis-multi-cluster-c-1" }]));
